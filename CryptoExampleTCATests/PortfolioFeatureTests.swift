...
extension BaseSuite {
    @MainActor
    @Suite struct PortfolioFeatureTests {

        // MARK: - AC1: coinTapped selects coin and clears amount (new coin)

        @Test func coinTappedSelectsCoinAndClearsAmount() async {
            let store = TestStore(
        ...
        @Test func coinTappedPrefillsExistingHolding() async {
        ...
        @Test func amountChangedUpdatesText() async {
            let store = TestStore(
        ...
        @Test func saveButtonTappedAddsNewHolding() async {
            let store = TestStore(
                initialState: PortfolioFeature.State(
                    coins: [Self.bitcoin],
                    selectedCoin: Self.bitcoin,
        ...
        @Test func saveButtonTappedUpdatesExistingHolding() async {
        ...
        @Test func saveButtonTappedNoOpsOnInvalidAmount() async {
            let store = TestStore(
                initialState: PortfolioFeature.State(
                    coins: [Self.bitcoin],
                    selectedCoin: Self.bitcoin,
        ...
        @Test func saveButtonTappedNoOpsOnAstronomicalAmount() async {
            let store = TestStore(
                initialState: PortfolioFeature.State(
                    coins: [Self.bitcoin],
                    selectedCoin: Self.bitcoin,
        ...
        @Test func saveButtonTappedNoOpsOnSubnormalAmount() async {
            let store = TestStore(
                initialState: PortfolioFeature.State(
                    coins: [Self.bitcoin],
                    selectedCoin: Self.bitcoin,
        ...
        @Test func saveButtonTappedAcceptsExactLowerBound() async {
            let store = TestStore(
                initialState: PortfolioFeature.State(
                    coins: [Self.bitcoin],
                    selectedCoin: Self.bitcoin,
        ...
        @Test func saveButtonTappedAcceptsExactUpperBound() async {
            let store = TestStore(
                initialState: PortfolioFeature.State(
                    coins: [Self.bitcoin],
                    selectedCoin: Self.bitcoin,
        ...
        @Test func saveButtonTappedRemovesExistingHoldingOnZeroAmount() async {
            await withDependencies {
                $0.realmController = .inMemory(id: UUID().uuidString)
            } operation: {
        ...
        @Test func saveButtonTappedRemovesExistingHoldingOnEmptyAmount() async {
            await withDependencies {
                $0.realmController = .inMemory(id: UUID().uuidString)
            } operation: {
                let seedStore = TestStore(
        ...
        @Test func saveButtonTappedRemovesOnlyTargetEntry() async {
            await withDependencies {
                $0.realmController = .inMemory(id: UUID().uuidString)
            } operation: {
                ...
                await seedEthereum.send(.saveButtonTapped) {
                    $0.$portfolioItems.withLock {
                        $0 = [PortfolioItem(coinID: "bitcoin", amount: 1.0), PortfolioItem(coinID: "ethereum", amount: 2.5)]
                    }
                    $0.amountText = ""
                    $0.selectedCoin = nil
                }
        ...
        @Test func saveButtonTappedNoOpsOnZeroAmountForNewCoin() async {
            let store = TestStore(
                initialState: PortfolioFeature.State(
                    coins: [Self.bitcoin],
                    selectedCoin: Self.bitcoin,
        ...
        @Test func saveButtonTappedNoOpsOnEmptyAmountForNewCoin() async {
            let store = TestStore(
                initialState: PortfolioFeature.State(
                    coins: [Self.bitcoin],
                    selectedCoin: Self.bitcoin,
